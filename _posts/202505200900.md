...
`@counter-style`を使うことで、CSSのみでリストカウンタの見た目を簡潔にカスタマイズできるようになる。特に、括弧やコロンといった装飾は、UIの文脈に応じてリストの可読性や印象を向上させる効果がある。`prefix`や`suffix`の組み合わせ次第で、ほかにもさまざまなパターンが表現可能であり、実務でも有用なテクニックである。ぜひ活用してみてほしい。
