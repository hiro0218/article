...
かつてはInternet Explorerが`Array.from`やSpread構文をサポートしておらず、Babelなどでトランスパイルが必要だった。しかし、モダンブラウザはこれらをネイティブでサポートしており、実装方法を簡略化できる。
...
Spread構文と`.map`の組み合わせは配列を2回走査する必要があるため、1回の走査で完了する`Array.from`の方が理論上は高速になる。しかし、Safariでの測定結果は予想に反しており、`.map`メソッドが内部的に最適化されている可能性がある。ただし、ベンチマークの実施方法やサンプルデータの特性による影響も考慮すべきであり、これが一貫した性能向上を意味するかは別途検証が必要である。
...
<!-- textlint-disable @textlint-ja/ai-writing/no-ai-emphasis-patterns -->
...
<!-- textlint-enable @textlint-ja/ai-writing/no-ai-emphasis-patterns -->
