...
Vuex は、[Flux](https://facebookarchive.github.io/flux/docs/in-depth-overview/)（＋[Redux](https://redux.js.org/)・[The Elm Architecture](https://guide.elm-lang.org/architecture/)）から影響を受けているため、Flux のアーキテクチャを知れば、Vuex を理解しやすくなるだろう。
...
[flux/examples/flux-concepts at main · facebookarchive/flux · GitHub](https://github.com/facebookarchive/flux/tree/main/examples/flux-concepts)

### Flux の構成要素とデータの流れ

Flux において、**データの流れは単一方向**である。

データの流れが単一方向という制約で構築することで、どこで状態の変更が起きているのか判別しやすくなり、またコンポーネント間で状態の変更を検知する必要がなくなり、状態管理が容易になる。
...
Dispatcher は、 Action を Store に送信する役割を持つ。Store は受け取った Action をもとに状態を更新し、Store に紐付いた View は画面を更新するといったフローになる。

データの流れは単一方向であるため、View から Store へ直接変更を加えることはできないが、View から Dispatcher に対して Action を渡すことで Store に変更を加えることはできる。「View から Store へ変更を加えなければならない」ケースでも単一方向という制約を破らないようにすることでデータフローを複雑化させることを避けられる。

---

### Action

Action は、アプリケーションの内部 API を定義する。アプリケーションとの相互作用をキャプチャし、「タイプ」フィールドといくつかのデータを持つ単純なオブジェクトである。
...
すべての Store が Action を受け取り、同じ`delete-user`Action を処理することで、「データをクリアする」「資格情報を更新する」必要性を把握できるようになる。

### Dispatcher

Dispatcher は、Action を受け取り、Dispatcher に登録されている Store を更新する。

Dispatcher は、アプリケーション内で単一であるため、すべての Action が必ず単一 Dispatcher を経由して Store を更新することが保証される。
...
[Vuex 公式ページに記載](https://vuex.vuejs.org/ja/#%E7%8A%B6%E6%85%8B%E7%AE%A1%E7%90%86%E3%83%91%E3%82%BF%E3%83%BC%E3%83%B3-%E3%81%A8%E3%81%AF%E3%81%AA%E3%82%93%E3%81%A7%E3%81%99%E3%81%8B%EF%BC%9F)されているカウンタアプリのサンプル：
...
しかしながら、上記のような状態管理は**複数のコンポーネントから State を共有する形**になると複雑さを増してしまう。

- **複数の View（コンポーネント）が、共通の State に依存する**
  プロパティ（props）として深く入れ子になったコンポーネントへ渡していくとコードの見通しが悪くなる（データをバケツリレーしていくことになるため）
...
2. 状態（State）は Mutations の `commit` による変更のみ可能である
...
Actions は非同期処理ができるため、そういった処理を含む場合は Actions に記述する。例えば、非同期でバックエンドとの通信（API）を伴うような処理が必要な場合に使用されることが多い。
...
タイプを指定して Mutations に登録してある関数を `store.commit()` 経由で呼び出す。
...
どういう処理をどちらに記述すれば良いのか。[公式](https://vuex.vuejs.org/ja/guide/mutations.html)には以下のように記載されている。
...
役割に応じて明示的に記述法が変わるため、コードの見通しが良くなる。
...
アプリケーションが複雑ではない場合（コンポーネント同士の状態が影響し合わない）、Vuex が不要なケースはある。

ただ、アプリケーションがシンプルな状態で完成する場合は未導入でも良いが、今後も成長する可能性があるのであれば、**Vuex は構築時点で導入しておく方が良い**。ゼロから導入する際のコストは、途中から導入するよりはるかに少ないし、Vue.js のエコシステム（[Vue CLI](https://cli.vuejs.org/) や [Nuxt.js](https://v2.nuxt.com/ja/guide/) など）の利用であればもっと簡単に導入ができるため、公式の引用にあるようなコストは掛からないだろう。
...
個人的には、「すべてのデータを Vuex で管理する」ことはお勧めしない。

そうした場合、規模感が大きくなるにつれ、Store の管理が複雑になってくる。コンポーネント内で `watch` や `commit` が走るとコンポーネント間の状態管理まで複雑化していく。
さらに複数人で開発していくと「状態」以外のもの（定数など）も State に定義し始めるケースも散見された（Store をグローバル変数として扱っているケース）。Store はシンプルな構成を維持するためにも、可能な限り「すべてのデータを Vuex で管理しないようにする」のが全体の見通しを良くできる。
...
> [!NOTE]
> **2026年現在の補足:**
> この記事は Vue 2 と Vuex（Vue 2 向けの状態管理ライブラリ）を前提に記載している。Vue 3 以降の状態管理について補足する。

- **Vuex → Pinia**: Vue 3 では、[Pinia](https://pinia.vuejs.org/) が公式推奨の状態管理ライブラリとなった。Pinia は本記事で述べた Mutations の概念を廃止し、Actions のみで状態変更を行うシンプルな設計を採用している。「Actions と Mutations の分離」は Pinia では不要になった
- **Vue CLI → Vite**: 本記事で言及した [Vue CLI](https://cli.vuejs.org/) はメンテナンスモードに移行した。新規プロジェクトでは [create-vue](https://github.com/vuejs/create-vue) + [Vite](https://vite.dev/) が推奨されている
- **Flux**: Flux プロジェクトは [2023年にアーカイブ](https://github.com/facebookarchive/flux)された。Flux の解説はアーキテクチャの理解として有用だが、積極的なメンテナンスは行われていない
