...
```text
1. 開発者Aが重複を発見する
2. 重複を抽出して新しい抽象化を作成する
3. 重複を新しい抽象化で置き換える
4. 時間が経過する
5. 新要件が発生、既存の抽象化が「ほぼ」適合する（が完全ではない）
6. 開発者Bがパラメータと条件分岐を追加する
7. さらに新要件が到着し、追加パラメータと条件分岐を追加する（繰り返し）
8. あなたが登場し、コードが理解不能な状態になっている
```
...
export const buildProductStrategy = (type: ProductType, options: StrategyOptions) => {
  ...
  if (type === 'sale' && options.highlight) return 'sale-highlight';
...
function renderButton(type: 'normal' | 'sale', highlight?: boolean) {
  if (type === 'sale' && highlight) return 'sale-highlight-button';
  if (type === 'sale') return 'sale-button';
