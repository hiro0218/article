...
[なぜタイポグラフィにおいて Vertical Rhythm（バーティカルリズム）は重要な手法なのか？ | デザイン | POSTD](http://postd.cc/why-vertical-rhythms/)に記載があった。（以下、引用）
...
[GitHub - modularscale/modularscale-sass: Modular scale calculator built into your Sass](https://github.com/modularscale/modularscale-sass) を見ると、Compass や Bower などインストール方法が色々とあるみたい。
...
`font-smoothing`は、フォントのアンチエイリアスを調整する CSS のプロパティである。独自実装であり、macOS の Chrome(webkit)/Firefox のみで使用できる。
