...
`div`要素は、デフォルトでインタラクティブな要素ではないため、スクリーンリーダーなどの支援技術から適切に認識されない。
...
`button`要素はデフォルトでフォーカス可能だが、`div`だとデフォルトではフォーカスされない。そのため、`tabindex="0"`を設定することでキーボードでのフォーカス移動が可能な状態にする。
...
`button`は`Enter`や`Space`キーで操作が可能だが、`div`にはその機能がない。
