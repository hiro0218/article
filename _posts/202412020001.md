...
代用するためにはボタン要素に`role="button"`を付与するだけでは解決せず、先述までのようにいくつも考慮する点がある。そのため、可能な限りネイティブのボタンを使用することが望ましい。ネイティブのボタンは、すべてのブラウザと支援技術でサポートされており、カスタマイズなしでキーボードとフォーカスの要件を満たしているからである。
