...
- **リンク確認できない環境**: `markdown-link-check` や WebFetch が使えない場合は、利用可能な Web 確認手段で代替する。Web 確認手段が使えない、ネットワーク制約で確認できない、または短時間で確認が終わらない場合は再試行を続けない。軸 9 は `0 (未検証)` とし、レポートに「リンク到達性は未検証」と明記する。未検証を「リンク切れ」とは書かない
