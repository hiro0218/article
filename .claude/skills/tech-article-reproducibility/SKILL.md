...
技術記事の品質を「読者が手元で同じことを再現できるか」の観点で測る。文体評価や論理評価とは独立した別軸。
...
- **軸 9 (リンク健全性)**: 全リンクが public に到達可能なら 2 点。到達確認できないリンクが一部あるが、リンク切れや認証必須とは断定できない場合は 1 点。確認手段が全く使えない、または主要リンクが未検証の場合は `0 (未検証)` とする。確認済みのリンク切れ、認証必須、private リンクが主要情報源に含まれる場合は 0 点。HTTP 到達性と public アクセス可否まで確認すれば 2 点に足り、コンテンツ検証は不要。確認手段がない、ネットワーク制約、短時間で終わらない場合は再試行せず `0 (未検証)` とし、リンク切れとは書かない。この記事リポジトリでは `/images/` 画像を含む記事で `npx markdown-link-check` が `ERR_INVALID_FILE_URL_HOST` で落ちる。CLAUDE.md『リンク検証の既知問題』の回避策で外部リンクだけ確認する
...
5. 詰まりポイントに対応する記述を記事に追加 / 修正。サンプルコードを全て inline にせず、リポジトリへのリンクと併用する
